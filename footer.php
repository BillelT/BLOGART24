<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="/src/css/footer.css">
</head>
<body>
    <footer>
        <nav>
            <h3>Menu</h3>
            <ul>
                <li>
                    <a href="/index.php">Accueil</a>
                </li>
                <li>
                    <a href="/views/frontend/articles/evenement.php?numArt=2">Articles évènement</a>
                </li>
                <li>
                    <a href="/views/frontend/articles/interview.php?numArt=1">Articles portrait</a>
                </li>
                <li>
                    <a href="#">Témoignages</a>
                </li>
                <li>
                    <a href="/views/frontend/articles/faq.php">FAQ</a>
                </li>
                <li>
                    <a href="/views/frontend/inscription.php" class="no-connected" id="noConnected">Inscription</a>
                </li>
                <li>
                    <a href="/views/frontend/connexion.php" class="no-connected" id="noConnected">Connexion</a>
                </li>
            </ul>
        </nav>
        <nav>
            <h3>Contact</h3>
            <ul>
                <li>
                    <a href="mailto:antoine.cordberand@mmibordeaux.com">antoine.corberand@mmibordeaux.com</a>
                </li>
                <li>
                    <a href="tel:+33647503400">06.47.50.34.00</a>
                </li>
                <li>
                    <a href="#">Bordeaux, France</a>
                </li>
            </ul>
        </nav>
        <nav>
            <h3>Sécurité</h3>
            <ul>
                <li>
                    <a href="/views/frontend/rgpd/Politiques.php">Politiques de confidentialité</a>
                </li>
                <li>
                    <a href="/views/frontend/rgpd/cgu.php">CGU</a>
                </li>
                <li>
                    <a href="/views/frontend/rgpd/Mentions.php">Mentions légales</a>
                </li>
            </ul>
        </nav>
        <img src="/src/images/footer logo bibbles.webp" alt="logo Bibble'S">
        <p>Tous droits réservés - 2024</p>
    </footer>
</body>
</html>